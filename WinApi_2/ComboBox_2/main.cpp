	...
	case WM_COMMAND:
		...
		case IDOK:
		{
			...
			sprintf(sz_message, "Вы выбрали пункт №%i со значением \"%s\".", i, sz_buffer);
				//Функция sprint_f() выполняет форматирование строк тоесть позволяет вставить в строку переменные значения
				//Спецификатор %i - это целое число 
				//Спецификатор %s - это строка
